...
Router.map(function() {
    this.route('testArea', {path: '/test', waitOn: function() { return [ Meteor.subscribe('accounts')] } });
    ...
    this.route('quotes.all', {path: '/quotes', waitOn: function() { return [ Meteor.subscribe('quotes') ]}});
    this.route('quote.select', {path: '/quote/select'});

    this.route('quotes.new', {
        path: '/quote/new/:accountId',
        waitOn: function() {
            return [
                Meteor.subscribe('products'),
                Meteor.subscribe('accounts')
        ...
        data: function() {
            return {
                account: Accounts.findOne({_id : this.params.accountId})
            }
        }
    });
